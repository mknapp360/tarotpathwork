export type ShemAngel = {
  number: number;
  name: string;
  guardian_dates: string[]; // e.g. ["March 21", ...]
  morality_dates: string[]; // e.g. ["March 20", "May 31", ...]
  soul_time: string[]; // e.g. ["00:00 - 00:19"] (not strictly needed if using 20-min blocks)
  description: string;
};
 
export const SHEM_ANGELS: ShemAngel[] = [
  // --- SAMPLE (first few from your Wix file) ---
  {
    "number": 1,
    "name": "Vehuiah",
    "guardian_dates": [
